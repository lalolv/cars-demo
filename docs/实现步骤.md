...
**light_pillar.tscn 结构：**

```
LightPillar (Node3D) [light_pillar.gd]
└── MeshInstance3D (CylinderMesh)
	└── 材质: StandardMaterial3D (emission 启用)
```
...
@export var frequency_range: Vector2 = Vector2(0, 100)  # Hz
...
func update_from_audio(magnitude: float) -> void:
	# magnitude: 0.0 ~ 1.0
	var height = base_height * (0.5 + magnitude * 0.5)
	mesh.scale.y = height / base_height
	mesh.position.y = height / 2.0

	# 亮度随音量变化
	material.emission_energy_multiplier = 1.0 + magnitude * 3.0
```
...
func _process(_delta: float) -> void:
	...
	var pillar_count = light_pillars.size()
	if pillar_count == 0:
		return

	# 将频率范围分配给各光柱
	var freq_step = (max_freq - min_freq) / pillar_count

	for i in range(pillar_count):
		var freq_low = min_freq + i * freq_step
		var freq_high = freq_low + freq_step
		...
		# 转换为 0-1 范围（对数缩放）
		magnitude = clamp((60 + linear_to_db(magnitude)) / 60.0, 0.0, 1.0)

		if light_pillars[i].has_method("update_from_audio"):
			light_pillars[i].update_from_audio(magnitude)
```
